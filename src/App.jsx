...
export default function HockeyTracker() {
  ...
  const [form, setForm] = useState({
    ...
    matchChoice: '__new__', newMatchLabel: '',
  });
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [playersRes, matchesRes, entriesRes, goalsRes] = await Promise.all([
        ...
        const loadedMatches = matchRows.map((r) => ({ id: r.id, date: r.date, label: r.label }));
  ...
  async function insertMatchRow(match) {
    const { error: err } = await supabase.from('matches').insert({ id: match.id, date: match.date, label: match.label });
  ...
  async function handleAddGame(e) {
    ...
    try {
      ...
      if (matchId === '__new__') {
        const label = form.newMatchLabel.trim() || `Игровая тренировка ${fmtDate(form.date)}`;
        const newMatch = { id: `m${Date.now()}`, date: form.date, label };
      ...
      setForm({ ...form, goals: '', assists: '', matchChoice: '__new__', newMatchLabel: '' });
  ...
  const seasonAwards = useMemo(() => {
    ...
    seasons.filter(isSeasonComplete).forEach((s) => {
      const gamesInSeason = entries.filter((e) => seasonLabel(e.date) === s);
  ...
  }, [entries, players, seasons]);
  const seasonWrapped = useMemo(() => {
    if (!currentSeason || seasonEntries.length === 0) return null;
    ...
    const bestGame = seasonEntries.reduce((best, e) => {
      const pts = e.goals + e.assists;
      return !best || pts > best.points ? { ...e, points: pts } : best;
    }, null);
    const totals = seasonEntries.reduce((acc, e) => {
      acc.goals += e.goals;
    ...
    if (prevSeason) {
      const prevEntries = entries.filter((e) => seasonLabel(e.date) === prevSeason);
    ...
    return { topScorer, topAssist, bestGame, totals, mostImproved, prevSeason, gamesCount: seasonGP };
  }, [currentSeason, seasonEntries, seasonStats, seasons, entries, players, seasonGP]);
